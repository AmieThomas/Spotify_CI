...
final <- select(final, -session_id, -track_id)
set.seed(592)
...
trainT_clean <- trainT[complete.cases(trainT), ]
trainC_clean <- trainC[complete.cases(trainC), ]

# Fit regression models
model1 <- glmnet(as.matrix(trainT_clean[, -which(names(trainT_clean) %in% c("cluster_similarity", "not_skipped"))]),
                 as.factor(trainT_clean$not_skipped), family = "binomial", alpha = 1)

model0 <- glmnet(as.matrix(trainC_clean[, -which(names(trainC_clean) %in% c("cluster_similarity", "not_skipped"))]),
                 as.factor(trainC_clean$not_skipped), family = "binomial", alpha = 1)
...
test_predictors <- as.matrix(test[, -which(names(test) %in% c("cluster_similarity", "not_skipped"))])

# Predictions for model0
u0 <- predict(model0, newx = test_predictors, type = "response")
